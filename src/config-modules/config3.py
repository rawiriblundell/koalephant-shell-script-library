...
fileName = sys.argv[2]
file = open(fileName, 'r+' if op == 'write' else 'r')

ini = io.StringIO('[' + FAKE_SECTION_NAME + ']\n' + file.read())
...
if op == 'sections':
	...
	tmpString = io.StringIO()  # type: StringIO
	config.write(tmpString)
	tmpString.seek(0)
	file.seek(0)
	file.write(tmpString.read().replace('[' + FAKE_SECTION_NAME + ']\n', ''))
	file.close()
